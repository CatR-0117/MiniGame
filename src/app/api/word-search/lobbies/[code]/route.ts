...
import {
  getWordSearchLobbyByCode,
  leaveWordSearchLobbyByCode,
...
export async function GET(request: Request, context: LobbyRouteContext) {
  ...
  const result = await getWordSearchLobbyByCode(code, playerId);
...
export async function DELETE(request: Request, context: LobbyRouteContext) {
  ...
  const result = await leaveWordSearchLobbyByCode(code, playerId, rejoinToken);
